...
public class ApiConnectorMock implements ApiConnector {
	...
	private static Class<? extends ApiObjectBase> getVncClass(String clsname) {
		...
		try {
			Class<? extends ApiObjectBase> cls = (Class<? extends ApiObjectBase>) Class
					.forName("net.juniper.contrail.api.types." + typename);
			return cls;
		} catch (Exception e) {
	...
	void buildDefaultConfig() {
		try {
			...
			HashMap<Class<? extends ApiObjectBase>, List<HashMap<String, ApiObjectBase>>> defaultConfigMap = (HashMap<Class<? extends ApiObjectBase>, List<HashMap<String, ApiObjectBase>>>) ois
					.readObject();
	...
	private boolean validate(ApiObjectBase obj) throws IOException {
		...
		if (parent != null) {
			try {
				s_logger.debug("Verify establish parent(" + _apiBuilder.getTypename(parent.getClass()) + ", "
						+ parent.getName() + ") => child (" + _apiBuilder.getTypename(obj.getClass()) + ", "
						+ obj.getName());
				...
				s_logger.debug("Verify Establish child(" + _apiBuilder.getTypename(obj.getClass()) + ", "
						+ obj.getName() + ") => backref to parent(" + _apiBuilder.getTypename(parent.getClass()) + ", "
						+ parent.getName() + ")");
			} catch (Exception e) {
	...
	public synchronized boolean create(ApiObjectBase obj) throws IOException {
		...
		if (parent != null) {
			try {
				s_logger.debug("Establish parent(" + _apiBuilder.getTypename(parent.getClass()) + ", "
						+ parent.getName() + ") => child (" + _apiBuilder.getTypename(obj.getClass()) + ", "
						+ obj.getName() + ")");
				/* update parent object with new child info */
	...
	ApiObjectBase getDefaultParent(ApiObjectBase obj) {
		...
		if (obj.getName().equals("default-domain"))
			return null;
	...
	@Override
	public synchronized ApiObjectBase find(Class<? extends ApiObjectBase> cls, ApiObjectBase parent, String name)
			throws IOException {
		s_logger.debug(
				"find(cls, parent, name) : " + _apiBuilder.getTypename(cls) + ", " + parent.getName() + ", " + name);
		List clsData = getClassData(cls);
		if (clsData == null) {
	...
	@Override
	public synchronized List<? extends ApiObjectBase> list(Class<? extends ApiObjectBase> cls, List<String> parent)
			throws IOException {
	...
	private boolean isChildrenExists(ApiObjectBase parent) {
		...
		ArrayList<List<HashMap<String, ApiObjectBase>>> clsDataList = new ArrayList<List<HashMap<String, ApiObjectBase>>>(
				_map.values());
	...
	private void updateRefsVerify(ApiObjectBase obj) throws IOException {
		...
		for (Field f : cls.getDeclaredFields()) {
			...
			for (ObjectReference<ApiPropertyBase> ref : nv) {
				...
				if (refObj == null) {
					s_logger.debug("Can not find obj for class: " + _apiBuilder.getTypename(refCls) + ", uuid: "
							+ ref.getUuid() + " , href: " + ref.getHRef());
					throw new IOException("Obj " + obj.getName() + " has a reference of type<"
							+ _apiBuilder.getTypename(refCls) + ", but object does not exist");
				}
				s_logger.debug("Verify establish backref on(cls, obj) : " + _apiBuilder.getTypename(refCls) + " => "
						+ refObj.getName() + " with ref(cls, obj): " + _apiBuilder.getTypename(cls) + ", "
						+ obj.getName());
	...
	private void updateRefs(ApiObjectBase obj) throws IOException {
		...
		for (Field f : cls.getDeclaredFields()) {
			...
			for (ObjectReference<ApiPropertyBase> ref : nv) {
				...
				s_logger.debug("Establish backref on(cls, obj) : " + _apiBuilder.getTypename(refCls) + " => "
						+ refObj.getName() + " with ref(cls, obj): " + _apiBuilder.getTypename(cls) + ", "
						+ obj.getName());
	...
	private void updateObjectVerify(ApiObjectBase obj, ApiObjectBase other, String fieldName) throws IOException {
		s_logger.debug("updateObjectVerify(cls, obj, other-cls, other, field): "
				+ _apiBuilder.getTypename(obj.getClass()) + ", " + obj.getName() + ","
				+ _apiBuilder.getTypename(other.getClass()) + ", " + other.getName() + "," + fieldName);
		Class<?> cls = obj.getClass();
	...
	private void updateObject(ApiObjectBase obj, ApiObjectBase other, String fieldName) throws IOException {
		s_logger.debug("updateObject(cls, obj, other-cls, other, field): " + _apiBuilder.getTypename(obj.getClass())
				+ ", " + obj.getName() + "," + _apiBuilder.getTypename(other.getClass()) + ", " + other.getName() + ","
				+ fieldName);
		Class<?> cls = obj.getClass();
